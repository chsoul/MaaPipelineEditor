...
export const updateLogs: UpdateLogItem[] = [
  ...
  },
  {
    ...
    updates: {
      features: [
        "🚨 调试面板提供字段级静态错误检查",
        "💊 新增灵动胶囊小组件，信息查看更便捷（现已接入调试等功能，支持快速停止等操作）",
        "🏆 新增成就系统，别笑，你也过不了第二关！",
        "🔔 调试面板支持节点通知",
        "🎨 新增操作时隐藏边渲染选项（默认关闭，可在设置中开启，开启后将在拖动画布、节点时大幅提升性能）",
      ],
      perfs: [
        ...
        "优化节点量较大时拖动画布的流畅度",
        "神经网络 expected 支持标签名和混合数组",
        "截图支持 Expand 模式",
        "Win32 控制器支持 AnchoredTouch 输入方式与独立配置键盘输入方式",
        "Linux 统一控制器支持 PipeWire 截图配置与 Libei 输入配置",
        "StopApp 支持留空以终止当前控制器窗口所属进程",
        "支持 focus.trace 字段编辑",
        "更新 PI 协议定义",
